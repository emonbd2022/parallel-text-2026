...
Do not include explanations, markdown, comments, or any additional text.`;
  ...
  try {
    ...
    if (!catText) throw (lastError || new Error("No response from AI"));
...
export const parseAndValidateCategoryResponse = (catText: string, items: { id: string }[]): Record<string, { category: string }> => {
    ...
    let cleanText = catText.replace(/```json/gi, "").replace(/```/g, "").trim();
    const match = cleanText.match(/\[[\s\S]*\]/);
    if (match) cleanText = match[0];
    
    let catArray: any[];
    try {
      catArray = JSON.parse(cleanText);
    } catch (e) {
      throw new Error(`INVALID_CATEGORY_RESPONSE: Failed to parse JSON: ${catText.slice(0, 100)}`);
    }
    ...
    catArray.forEach(catItem => {
      ...
      if (typeof rawCategory !== 'string') {
        rawCategory = String(rawCategory || 'Graphic Resources');
      }
      ...
      let canonicalCategory = AUTHORITATIVE_CATEGORIES.find(c => c.toLowerCase() === normalizedInput);

      if (!canonicalCategory && CATEGORY_SYNONYMS[normalizedInput]) {
        canonicalCategory = CATEGORY_SYNONYMS[normalizedInput];
      }

      if (!canonicalCategory) {
        const partialMatch = AUTHORITATIVE_CATEGORIES.find(c => normalizedInput.includes(c.toLowerCase()) || c.toLowerCase().includes(normalizedInput));
        if (partialMatch) {
          canonicalCategory = partialMatch;
        }
      }

      if (!canonicalCategory) {
        console.warn(`[Category Warning] Index: ${idx} Gemini Raw: "${catItem.category}" mapped to fallback "Graphic Resources"`);
        canonicalCategory = "Graphic Resources";
      }
    ...
    // Fill in any missing indices with sensible default
    for (let i = 0; i < items.length; i++) {
      if (!seenIndices.has(i)) {
        const missingId = items[i].id;
        results[missingId] = { category: "Graphic Resources" };
      }
    }
