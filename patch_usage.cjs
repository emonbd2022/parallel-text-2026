...
function getUsageDocId(uid, date) {
    return crypto.createHmac('sha256', keyBuffer).update(uid + '-' + date).digest('hex');
}
...
content = content.replace('apiRouter.post("/central-generate"', code + 'apiRouter.post("/central-generate"');
