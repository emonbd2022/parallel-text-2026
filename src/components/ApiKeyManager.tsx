            ...
            <div key={k.id} className={`flex items-center justify-between transition-colors p-3 rounded-xl border group relative
              ...
              <div className="flex flex-col overflow-hidden mr-3 flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                   ...
                   {/* Usage Badges */}
                   <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono font-bold flex items-center gap-1 ${flash_3_Limit ? 'bg-red-500/20 text-red-400' : 'bg-cyan-500/10 text-cyan-400'}`} title="3 Flash Requests Today">
                      🔥 {usage.flash_3}/20
                   </span>
                   <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono font-bold flex items-center gap-1 ${flashLimit ? 'bg-red-500/20 text-red-400' : 'bg-cyan-500/10 text-cyan-400'}`} title="2 Flash Requests Today">
                      ⚡ {usage.flash}/20
                   </span>
                   <span className={`text-[9px] px-1.5 py-0.5 rounded font-mono font-bold flex items-center gap-1 ${liteLimit ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/10 text-amber-400'}`} title="Lite Requests Today">
                      💡 {usage.lite}/20
                   </span>
